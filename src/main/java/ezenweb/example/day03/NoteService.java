...
public class NoteService {
    ...
    public boolean update(NoteDto noteDto){log.info("get update in : " + noteDto);
        // 1. 수정할 pk번호를 이용한 엔티티 검색
        Optional<NoteEntity> optionalNoteEntity = noteEntityRepository.findById(noteDto.getNno());
        // 2. 포장클래스<엔티티>
        if(optionalNoteEntity.isPresent()){ // 포장클래스내 엔티티 들어가있으면
            NoteEntity noteEntity = optionalNoteEntity.get(); // 엔티티 꺼내기
