...
public class ProductEntity extends BaseTime{
    @Id private String id;                              // 제품번호 [ JPA는 1개 이상의 ID(PK) 필요! ] / 오토키X -> 오늘날짜/밀리초
    @Column(nullable = false) private String pname;     // 제품명 [JPA로 DB 필드 선언시 _ 사용 X]
    ...
    // 제품이미지 [1:다] 연관관계 [*추후]
    // pk필드 선언시 // mappedBy="참조할필드명" // 제약조건 : pk객체가 삭제되면 fk객체에 제약조건
    ...
    // 1. 출력용 [관리자가 보는 입장 - 관리자 페이지에서 출력용]
...
/*
    제약조건 : pk객체가 삭제되면 fk객체에 제약조건
        cascade = CascadeType.ALL	: pk객체 삭제/수정시 fk객체 모두 삭제/수정
        ...
        cascade = CascadeType.DETACH	: pk객체 삭제/수정의 변경사항 fk에 반영 안함
*/
