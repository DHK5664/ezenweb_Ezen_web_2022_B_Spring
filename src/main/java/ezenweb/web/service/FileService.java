...
public class FileService {
    ...
    // 1. 업로드 , 다운로드
    //String path = "C:\\java\\"; // spring서버 로컬드라이브[c] 접근가능
    // 2. 업로드 , 다운로드 , 리액트 리소스 접근
    // JS[react] 로컬드라이브[c] 접근 불가능--> 리액트서버에 업로드
    // springboot + react 통합
    // react build --> spring resources --> spring build --> 프로젝트내 build
    String path = "C:\\Users\\504\\Desktop\\ezenweb_Ezen_web_2022_B_Spring\\build\\resources\\main\\static\\static\\media\\";
    ...
    public FileDto fileupload( MultipartFile multipartFile){
        ...
        log.info("File upload : " + multipartFile.getSize());               // 바이트 단위

        // 1. 첨부파일 존재하는지 확인
        if(!multipartFile.getOriginalFilename().equals("")){ // 첨부파일이 존재하면

            // * 만약에 다른 이미지인데 파일명이 동일하면 덮어씌워짐[ 식별 불가 ] : UUID + 파일명
            String fileName =
    ...
    @Autowired
    private HttpServletResponse response; // 응답 객체 (컨트롤에서는 @RestController 이 친구가 해줘서 입력 안했음)
    
    public void filedownload(String uuidFile){ // spring 다운로드 관련 API 없음
        ...
        try {
            ...
            // 2. 다운로드 스트림 구성
            File file = new File(pathFile); // 다운로드 할 파일의 경로에서 파일객체화
            ...
            // 4.스트림 [ 서버가 읽어온 바이트를 출력할 스트림 = 대상 : response = 현재 서비스 요청한 클라이언트에게]
            BufferedOutputStream fout = new BufferedOutputStream(response.getOutputStream());
            fout.write(bytes); // 입력스트림에서 읽어온 바이트 배열을 내보내기
            fout.flush(); // 스트림 메모리 초기화
            fin.close(); fout.close(); // 스트림 안전하게 닫기
        }catch (Exception e){
